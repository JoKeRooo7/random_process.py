...
class BoxApp:
    ...
    def check_unstable_equilibrium(self):
        return self.box.n_left == self.box.n // 2

    def check_equilibrium(self, stop_after_corresponding):
        if self.check_unstable_equilibrium():
            self.equilibrium += 1
        if self.equilibrium == stop_after_corresponding:
            return True
        self.equilibrium = 0
        return False
    ...
    def main(self, stop_after_corresponding=3,step=10000000, visual=False):
        ...
        for _ in range(step):
            ...
            if self.check_equilibrium(stop_after_corresponding):
                break
...
def main():
    # my_box_8 = create_box(8, visual=True)
...
if __name__ == "__main__":
    # calculate_unstable_equilibrium_data_and_save_data()
    main()